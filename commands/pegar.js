	...
	if (message.guild) {
		...
		if (message.author == args[0]) {
			const embed = new Discord.MessageEmbed()
				.setColor(0xefb810)
				.setDescription(`${message.author} se está pegando solo.`)
			...
			const embed = new Discord.MessageEmbed()
				.setColor(0xefb810)
				.setDescription(`${message.author} está pegando al aire.`)
				.setImage(randomItem(images));
      ...
      if (!user) return message.reply("Por favor, menciona a alguien.");
			const embed = new Discord.MessageEmbed()
				.setColor(0xefb810)
				.setDescription(`${message.author} está pegando a ${mention}.`)
				.setImage(randomItem(images));
